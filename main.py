...
class ProtocolError(Exception):
    ...
    def __repr__(self):
        return "<ProtocolError for %s: %s %s>" % (self.url, self.code, self.message)
...
class Date:
    ...
    def __repr__(self):
        return "<Date %s at %x>" % (time.asctime(time.localtime(self.value)), id(self))
...
class HessianWriter:
    ...
    def write_object(self, value):
        try:
            ...
            raise Exception("cannot write %s objects" % type(value))
        else:
            f(self, value)
...
class HessianParser:
    ...
    def parse_object_code(self, code):
        ...
        if code == 'N':
            ...
            raise Exception("UnknownObjectCode %s" % code)
...
class Hessian:
    ...
    def __invoke(self, method, params):
        ...
        r = requests.post(self._url, data=request)
        return self.parse_response(io.BytesIO(r.content))
